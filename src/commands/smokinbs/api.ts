...
export default class API extends Command {
	...
	async run() {
		...
		const zip = fs.createWriteStream(
			`SmokinBs-API${flags.testing ? "-testing" : ""}.zip`
		);
		await fetch(
			`https://api.smokinbsbbq.tk/v0/downloads?file=api&testing=${flags.testing}`,
			{
				headers: {
			...
			res.body?.on("end", () => {
				...
				spinner_download.success({
					text: `Downloaded SmokinBs-API to ${process.cwd()}/SmokinBs-API${
				...
				const extract = new AdmZip(
					`${process.cwd()}/SmokinBs-API${
						flags.testing ? "-testing" : ""
					}.zip`
				);
				extract.extractAllTo(
					process.cwd() +
						`/SmokinBs-API${flags.testing ? "-testing" : ""}`,
					true
				);
