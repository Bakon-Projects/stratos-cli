...
export default class FoodTruck extends Command {
	static flags = {
	...
	async run() {
		const { flags } = await this.parse(FoodTruck);
		...
		const zip = fs.createWriteStream(
			`SmokinBs-FoodTruck${flags.testing ? "-testing" : ""}.zip`
		);
		await fetch(
			`https://api.smokinbsbbq.tk/v0/downloads?file=foodtruck&testing=${flags.testing}`,
			{
				headers: {
			...
			res.body?.on("end", () => {
				...
				spinner_download.success({
					text: `Downloaded SmokinBs-FoodTruck to ${process.cwd()}/SmokinBs-FoodTruck${
				...
				const extract = new AdmZip(
					`${process.cwd()}/SmokinBs-FoodTruck${
						flags.testing ? "-testing" : ""
					}.zip`
				);
				extract.extractAllTo(
					process.cwd() +
						`/SmokinBs-FoodTruck${flags.testing ? "-testing" : ""}`,
					true
				);
